          ...
          child: Column(
            ...
            children: [
              Text(
                ...
                style: const TextStyle(fontSize: 52, fontWeight: FontWeight.w800, color: Color(0xFF2D3436), letterSpacing: -1),
              ),
              Row(
                mainAxisAlignment: MainAxisAlignment.center,
              ...
              Text('$reviewCount đánh giá', style: TextStyle(color: Colors.grey.shade500, fontSize: 13, fontWeight: FontWeight.w500)),
            ],
          ),
        ),
              ...
              return Column(
                children: [5, 4, 3, 2, 1].map((star) {
                  return Padding(
                    ...
                    child: _StarProgressRow(star: star, value: total == 0 ? 0 : (counts[star]! / total)),
                  );
                }).toList(),
              );
            },
...
class _StarProgressRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        SizedBox(width: 10, child: Text('$star', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.black54))),
            ...
            child: FractionallySizedBox(
              ...
              child: Container(
                decoration: BoxDecoration(
                  ...
                  gradient: LinearGradient(colors: [Colors.amber, Colors.amber.shade300]),
                ),
              ),
            ),
          ),
...
class _ReviewItem extends StatelessWidget {
  ...
  const _ReviewItem({
    ...
    required this.reviewText, required this.createdAt, required this.mediaUrls, this.userImage,
  });
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(timeago.format(createdAt), style: TextStyle(fontSize: 12, color: Colors.grey.shade400)),
                  ],
                ),
              ),
              if (isOwner)
          ...
          const SizedBox(height: 16),
          Row(
            children: List.generate(5, (index) => Icon(index < rating ? Icons.star_rounded : Icons.star_outline_rounded, size: 18, color: Colors.amber)),
          ),
          const SizedBox(height: 12),
          if (title.isNotEmpty) Padding(
            padding: const EdgeInsets.only(bottom: 6),
            child: Text(title, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 16, color: Color(0xFF2D3436))),
          ),
          Text(reviewText, style: TextStyle(height: 1.6, color: Colors.grey.shade700, fontSize: 14)),
          const SizedBox(height: 16),
            ...
            SizedBox(
              ...
              child: ListView.separated(
                ...
                itemBuilder: (context, index) => ClipRRect(
                  ...
                  child: Image.network(mediaUrls[index], width: 100, height: 100, fit: BoxFit.cover),
                ),
              ),
            ),
          const SizedBox(height: 16),
            ...
            children: [
              if (isOwner && !isApproved)
                Container(
                  ...
                  child: const Text("⏳ Đang chờ duyệt", style: TextStyle(color: Colors.orange, fontSize: 11, fontWeight: FontWeight.bold)),
                )
              else const SizedBox(),
              InkWell(
                onTap: () {},
                child: Row(
                  children: [
                    ...
                    Text("Hữu ích", style: TextStyle(color: Colors.grey.shade500, fontSize: 13)),
                  ],
                ),
              ),
            ],
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              await FirebaseFirestore.instance.collection('reviews').doc(reviewId).update({'isDeleted': true, 'updatedAt': Timestamp.now()});
