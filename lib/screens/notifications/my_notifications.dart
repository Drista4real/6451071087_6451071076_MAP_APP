...
class MyNotificationScreen extends StatelessWidget {
  ...
  // Hàm hỗ trợ lấy Icon và Màu sắc dựa trên nội dung thông báo
  Map<String, dynamic> _getStatusStyle(String message) {
    String msg = message.toLowerCase();
    if (msg.contains('created')) return {'icon': Icons.create, 'color': Colors.blue};
    if (msg.contains('pending')) return {'icon': Icons.hourglass_empty, 'color': Colors.orange};
    if (msg.contains('processing')) return {'icon': Icons.sync, 'color': Colors.amber};
    if (msg.contains('shipped')) return {'icon': Icons.local_shipping, 'color': Colors.purple};
    if (msg.contains('delivered')) return {'icon': Icons.check_circle, 'color': Colors.green};
    if (msg.contains('cancelled')) return {'icon': Icons.cancel, 'color': Colors.red};
    if (msg.contains('returned')) return {'icon': Icons.keyboard_return, 'color': Colors.blueGrey};
    if (msg.contains('refunded')) return {'icon': Icons.monetization_on, 'color': Colors.teal};
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () {
              // Bạn có thể thêm logic đánh dấu đã đọc tất cả ở đây
            },
          ),
        ],
      ),
                      ...
                      Expanded(
                        child: ListTile(
                          ...
                          onTap: () async {
                            ...
                            // Lấy chi tiết đơn hàng để chuyển màn hình
                            ...
                            if (orderSnapshot.docs.isNotEmpty) {
                              ...
                              data['docId'] = orderSnapshot.docs.first.id;
