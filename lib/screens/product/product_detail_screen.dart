...
import 'package:ltud_lab/screens/review/write_review_screen.dart';
...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                          ...
                          children: [
                            Row(
                              children: [
                                ...
                                Text(product.brandName ?? '', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                        ...
                        Text("\$${product.price.toStringAsFixed(0)}", style: TextStyle(fontSize: 28, fontWeight: FontWeight.w900, color: Colors.blue.shade800)),
                        const SizedBox(height: 12),
                        _buildStockStatus(isOutOfStock),
                        const Divider(height: 40),
                        _buildReviewActionSection(product),
                        const SizedBox(height: 10),
                        ...product.attributes.map((attribute) => _buildAttributeSection(attribute)),
  ...
  Widget _buildThumbnails(dynamic product) {
    final List<String> allImages = [product.thumbnail, ...product.images];
    return SizedBox(
      height: 70,
  ...
  Widget _buildRatingBadge(dynamic product) {
    return GestureDetector(
      onTap: () => Get.to(() => ReviewRatingScreen(productId: product.id, rating: product.rating, reviewCount: product.ratingCount)),
      child: Container(
        ...
        child: Row(
          children: [
            ...
            const SizedBox(width: 4),
            Text("${product.rating}", style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.orange)),
            Text(" (${product.ratingCount})", style: TextStyle(color: Colors.orange.shade300, fontSize: 12)),
          ],
        ),
      ),
    );
  ...
  Widget _buildStockStatus(bool isOutOfStock) {
    return Container(
      ...
      decoration: BoxDecoration(color: isOutOfStock ? Colors.red.shade50 : Colors.green.shade50, borderRadius: BorderRadius.circular(8)),
      child: Text(
        isOutOfStock ? "Tạm hết hàng" : "Đang còn hàng",
        style: TextStyle(color: isOutOfStock ? Colors.red : Colors.green, fontWeight: FontWeight.w600, fontSize: 12),
      ),
    );
  }
  ...
  Widget _buildAttributeSection(dynamic attribute) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(name.toUpperCase(), style: const TextStyle(fontWeight: FontWeight.bold, letterSpacing: 1, fontSize: 13)),
        const SizedBox(height: 12),
        Wrap(
          spacing: 12,
          runSpacing: 12,
          children: List.generate(values.length, (index) {
            ...
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                child: Text(values[index], style: TextStyle(color: isSelected ? Colors.white : Colors.black87, fontWeight: isSelected ? FontWeight.bold : FontWeight.normal)),
              ),
            );
          }),
        ),
      ...
      builder: (context, snapshot) {
        ...
        return Padding(
          ...
          child: OutlinedButton.icon(
            onPressed: () => Get.to(() => WriteReviewScreen(product: product, reviewId: data["reviewId"])),
            style: OutlinedButton.styleFrom(
              padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
              side: BorderSide(color: Colors.blue.shade700),
            ),
            icon: Icon(state == "can_edit" ? Icons.edit : Icons.rate_review, size: 18),
            label: Text(state == "can_edit" ? "Chỉnh sửa đánh giá" : "Viết đánh giá sản phẩm"),
          ),
        );
      },
    );
                ...
                return ElevatedButton(
                  onPressed: isAdded ? null : () {
                    cartController.addToCart(CartItemModel(
                      productId: product.id, quantity: quantity, image: selectedImage,
                      price: product.price, title: product.title, brandName: product.brandName,
                  ...
                  child: Text(isAdded ? "ĐÃ Ở TRONG GIỎ" : "THÊM VÀO GIỎ HÀNG", style: const TextStyle(fontWeight: FontWeight.bold, letterSpacing: 1)),
                );
              }),
            ),
          ],
  ...
  Future<Map<String, dynamic>> getReviewState(String productId) async {
    ...
    if (user == null) return {"state": "not_allowed"};
    final purchased = await orderController.orderService.hasUserPurchasedProduct(userId: user.id, productId: productId);
    if (!purchased) return {"state": "not_allowed"};
    final reviewed = await orderController.hasUserReviewedProduct(userId: user.id, productId: productId);
    if (reviewed) {
      final snapshot = await FirebaseFirestore.instance
          ...
          .limit(1).get();
      return {"state": "can_edit", "reviewId": snapshot.docs.isNotEmpty ? snapshot.docs.first.id : null};
    }
